...
// RISC-V optimized rotations (if available)
#ifdef RISCV_RVC
#define RISC_V_ROTR(x, n) __builtin_bswap32(__builtin_rotateleft32(__builtin_bswap32(x), n))
#else
#define RISC_V_ROTR(x, n) ROTR(x, n)
#endif
...
void sha256_final(sha256_context_t* ctx, uint8_t* hash) {
    ...
    if (ctx->buffer_len > 56) {
        ...
        sha256_update(ctx, ctx->buffer, SHA256_BLOCK_SIZE);
        ctx->buffer_len = 0;
    }
...
// RISC-V optimized versions (simplified for now)
